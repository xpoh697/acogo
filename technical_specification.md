...
- *Аппаратные ограничения*: Функция поддерживается исключительно моделями серии PRO (модели 65, 67, 68) при подключении дополнительного аппаратного модуля коммутатора камер `PRO-VIDEO-SW2-60` и активном флаге конфигурации `cameraSwitching: true`. Для базовых моделей (модель 64) облако отклоняет запрос со статусом `401 Unauthorized`. В интеграции кнопка создается только для PRO-устройств, а ошибки облака обрабатываются без ложной ротации токенов сессии.
